...
for s in sources:
    ...
    df_trust.loc[s] =sub_tt[-1]
for col in df_trust:
    df_top = df_trust[col].copy()
    df_top10[col] = df_top.sort_values(ascending=False).index
df_top10 = df_top10.iloc[0:9,:]
df_top10.index += 1
print("In which topic do you want to see the ranking "
      "of the top 10 most reliable sources?\nChoose between",topics)

while flag == 1:
    x =input()

    if x in topics:
        print(df_top10[x].to_string())
        flag = 0
    else:
        print ("\nYou have typed incorrectly, try again (",topics,")")
