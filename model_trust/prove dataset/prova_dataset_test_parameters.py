...
for j in range(df1.shape[0]):
    ...
    trust_native1 = model_trust.compute_trust_different_config(expertise=expertise_native, goodwill=goodwill_native,
                                                               ...
                                                               alfa=1/3, beta=1/3, vartheta=1/3)
    ...
    Tfake2.append(*trustfake2.values())
    Tfake3.append(*trustfake3.values())
    Tfake4.append(*trustfake4.values())
...
"-------aggregation of same behaviour with different configuration-------"
# axs[0].plot(x1, T_native1)
# axs[0].plot(x1, T_native2)
# axs[0].plot(x1, T_native3)
# axs[0].plot(x1, T_native4)
# # axs[0].set_yticks(np.arange(0, 1, step=0.1))
# axs[0].set_xticks(np.arange(0, 170, step=10))
# axs[0].set_title('Benevolent behaviour')
# axs[1].plot(x1, Tfake1)
# axs[1].plot(x1, Tfake2)
# axs[1].plot(x1, Tfake3)
# axs[1].plot(x1, Tfake4)
# # axs[1].set_yticks(np.arange(0, 1, step=0.1))
# axs[1].set_xticks(np.arange(0, 170, step=10))
# axs[1].set_title('Malevolent behaviour')
...
for s in step:
    ...
    "-------aggregation of same behaviour with different configuration-------"
    # axs[step.index(s)+2].plot(x, T1)
    # axs[step.index(s)+2].plot(x, T2)
    # axs[step.index(s)+2].plot(x, T3)
    # axs[step.index(s)+2].plot(x, T4)
    # # axs[step.index(s)+2].set_yticks(np.arange(0, 1, step=0.1))
    # axs[step.index(s)+2].set_xticks(np.arange(0, 170, step=10))
    # axs[step.index(s)+2].set_title('OOA behaviour with semiperiod = '+str(s))
...
for ax in axs.flat:
    ...
    # ax.legend(['Trust with α =1/3, β = 1/3, ϑ = 1/3', 'Trust with α =0.7, β = 0.15, ϑ = 0.15',
    #            'Trust with α =0.15, β = 0.7, ϑ = 0.15', 'Trust with α =0.15, β = 0.15, ϑ = 0.7'],
    #           loc='center left', bbox_to_anchor=(1, 0.5))
