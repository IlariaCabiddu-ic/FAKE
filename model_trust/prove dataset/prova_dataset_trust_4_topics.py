...
# df = df[df['Source'] == 'breitbart']
# df = df[df['Topic'] == 'Politics']
list_sources = ['nytimes','nytimes','breitbart', 'breitbart']
list_topics = ['Politics', 'Crime & security','Politics', 'Business & Economy']
"-----------------analysis of each present source--------------"

sources = list(df['Source'].unique())
topics = list(df['Topic'].unique())

for s, topidx in zip(list_sources, list_topics):
    ordered_trust = {}
    print(s, topidx)
    sub_et = []
    sub_gt = []
    ...
    sub_df = sub_df[sub_df['Topic'] == topidx]  # iteration for the each source
    ...
    x = topics.index(topidx)
    t = sub_t[topics.index(topidx)]
    xt = np.arange(0, len(t))
    plt.plot(xt, t)
    plt.ylim([0, 1])
    plt.yticks(np.arange(0, 1, step=0.1))

    plt.xlabel('News requests during the time')
    plt.ylabel('Trust value')
    # ax.legend(df['Topic'].unique(), loc='lower right')
    list_legend = []
for s, top_idx in zip(list_sources,list_topics):
    list_legend.append((s,top_idx))
plt.legend(list_legend, loc='lower right')
plt.show()
